...
test.describe('Reports — Advanced Calendar & Filters', () => {
  ...
  test('custom date range selection and report filters update the data meaningfully', async ({ page }) => {
    ...
    await openCategoryFilter(page)
    const categoryToggleAll = page.getByTestId('reports-category-filter-toggle-all')
    await expect(categoryToggleAll).toContainText('Seleccionar todas')
    await expect(categoryToggleAll).toHaveAttribute('data-all-categories-selected', 'false')
    await screenshot(page, 'reports-category-multiselect-03-select-all-control')
    await categoryToggleAll.click()
    await page.getByTestId('reports-category-filter-trigger').click()
    await expect(page.getByTestId('reports-category-filter-trigger')).toContainText('Todas las categorías')
    await expect(page.getByTestId('reports-category-filter-trigger')).toHaveAttribute('data-all-categories-selected', 'true')
    await expectMovementCount(page, 4)

    await openCategoryFilter(page)
    await expect(categoryToggleAll).toContainText('Deseleccionar todas')
    await expect(categoryToggleAll).toHaveAttribute('data-all-categories-selected', 'true')
    await screenshot(page, 'reports-category-multiselect-04-deselect-all-control')
    await categoryToggleAll.click()
    await page.getByTestId('reports-category-filter-trigger').click()
    await expect(page.getByTestId('reports-category-filter-trigger')).toContainText('Sin categorías')
    await expect(page.getByTestId('reports-category-filter-trigger')).toHaveAttribute('data-selected-category-count', '0')
    await expect(page.getByText('Sin datos en este período')).toBeVisible({ timeout: REPORT_TIMEOUT })
    await screenshot(page, 'reports-category-multiselect-05-none-selected')
