...
import { movements, transfers } from '@/lib/db'

export function movementIsTransferSql() {
  return sql`(
    EXISTS (SELECT 1 FROM ${transfers} WHERE ${transfers.sourceMovementId} = ${movements.id})
    OR EXISTS (SELECT 1 FROM ${transfers} WHERE ${transfers.destinationMovementId} = ${movements.id})
  )`
}
