...
export function EmergencyDetailClient({ emergency, accounts }: Props) {
  ...
  async function handleDirectSettle() {
    const confirmed = window.confirm('¿Seguro que quieres marcar esta emergencia como saldada?')
    if (!confirmed) return
            ...
            <>
              ...
              <button
                ...
                style={{
                  ...
                  fontSize: 15,
                  fontWeight: 600,
                  cursor: settlingDirect ? 'not-allowed' : 'pointer',
                }}
              >
                {settlingDirect ? 'Procesando...' : '✅ Marcar como saldado'}
              </button>
            </>
          )}
